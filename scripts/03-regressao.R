...
cr <- read_csv2("http://leg.ufpr.br/~walmes/data/cristo-rei.csv")
rb <- read_csv2("http://leg.ufpr.br/~walmes/data/reboucas.csv")
str(cr)
...
da <- bind_rows(list(CRei = cr, Rebo = rb), .id = "bair")
da <- subset(da, tam > 10)
str(da)
...
ggplot(data = da,
       mapping = aes(x = tam, y = valor, color = bair)) +
    geom_point() +
    geom_smooth()
...
m0 <- lm(valor ~ tam, data = filter(da, bair == "Rebo"))
...
# Lambda da transformação Box-Cox.
MASS::boxcox(m0)
abline(v = 0, col = "red")
...
pred <- with(da, expand.grid(bair = unique(bair),
                             quar = 2,
                             banh = 2,
...
p1 +
    latticeExtra::as.layer(p2) +
    latticeExtra::as.layer(p3) +
    latticeExtra::layer(panel.text(x = 20, y = 10, label = eqn, bty = "n"))
...
pred <- data.frame(A = seq(0, 0.5, by = 0.025))

m0_dados <- lm(Gain ~ A + I(A^2), data = turk0)
summary(m0_dados)

turk0_m <- turk0 %>%
    group_by(A) %>%
    summarise(Gain = mean(Gain), n = n())
turk0_m

m0_media <- lm(Gain ~ A + I(A^2), data = turk0_m)
summary(m0_media)

pred_dados <- cbind(pred,
                    as.data.frame(predict(m0_dados,
                                          newdata = pred,
                                          interval = "confidence")))
pred_media <- cbind(pred,
                    as.data.frame(predict(m0_media,
                                          newdata = pred,
                                          interval = "confidence")))

ggplot(data = pred_dados,
       mapping = aes(x = A, y = fit)) +
    geom_line() +
    geom_ribbon(mapping = aes(ymin = lwr, ymax = upr),
                fill = "red",
                alpha = 0.3) +
    geom_line(data = pred_media, mapping = aes(x = A, y = fit), color = "orange") +
    geom_ribbon(data = pred_media,
...
xyplot(Gain ~ A, data = turk0) +
    latticeExtra::layer(panel.curve(th0 + th1 * x/(th2 + x)),
          data = start)
...
start <- list(th0 = 75, th1 = 0.7, th2 = 50)
xyplot(C ~ Temp, data = segreg) +
    latticeExtra::layer(panel.curve(th0 + th1 * (x - th2) * (x >= th2) +
                      0 * (x < th2)), data = start)
